...
protocol TeacherNetworkProtocol {
    func getTeachers(search: String, page: Int) throws -> AnyPublisher<[TeacherFullName], Error>
...
final class TeacherNetworkService: TeacherNetworkProtocol {
    ...
    func getTeachers(search: String, page: Int) throws -> AnyPublisher<[TeacherFullName], any Error> {
        guard let url = TeacherEndpoints.teachers(search).absoluteURL else {
            throw APIError.invalidResponse
        }

        return try network.fetch(url, [TeacherFullName].self)
    }
